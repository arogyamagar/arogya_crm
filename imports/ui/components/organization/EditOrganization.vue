            ...
            <form class="space-y-6" action="" @submit.prevent="handleSubmit">
                <div>
                    ...
                    <input
                        ...
                        required
                        v-model="modal.organization.name"
                    />
                </div>
                <div>
                    <label
                    ...
                    <input
                        ...
                        required
                        v-model="modal.organization.email"
                    />
                </div>
                <div>
                    <label
                    ...
                    <input
                        ...
                        required
                        v-model="modal.organization.address"
                    />
                </div>
                <div>
                    <label
                    ...
                    <input
                        ...
                        required
                        v-model="modal.organization.phone"
                        max="13"
                        min="10"
                    />
                </div>
...
export default {
    ...
    created() {
        ...
        this.getOrganizationData(this.$route.params.id)
    },
    meteor: {
        currentUser() {
            return Meteor.user()
    ...
    methods: {
        getOrganizationData(organizationId) {
            let data = OrganizationsCollection.findOne({ _id: organizationId })
            return data
            console.log(data)
        },
        async handleSubmit() {
            try {
                await Meteor.call('handelSubmit', organization._id, {
                    name: this.name,
                    email: this.email,
